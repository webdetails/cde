...
public class CdfRunJsDataSourceComponentWriter extends JsWriterAbstract implements IThingWriter {
  ...
  public void write( StringBuilder out, CdfRunJsDashboardWriteContext context, DataSourceComponent comp )
    ...
    if ( dataAccessId != null ) {
      ...
      } else if ( metaType.equals( META_TYPE_SOLR ) ) {
  ...
  private void renderCdaDatasource( StringBuilder out, DataSourceComponent dataSourceComp, String dataAccessId,
    ...
    addFirstJsProperty( out, PropertyName.DATA_ACCESS_ID, buildJsStringValue( dataAccessId ), INDENT2 );

    String outputIndexId = dataSourceComp.tryGetPropertyValue( PropertyName.OUTPUT_INDEX_ID, null );
    if ( outputIndexId != null ) {
      addJsProperty( out, PropertyName.OUTPUT_INDEX_ID, buildJsStringValue( outputIndexId ), INDENT2 );
    }
    ...
    if ( cdaPath != null ) {
      ...
      addJsProperty( out, PropertyName.PATH, buildJsStringValue( cdaPath ), INDENT2 );

    } else {

      // legacy
      String solution = dataSourceComp.tryGetPropertyValue( PropertyName.SOLUTION, "" );
      addJsProperty( out, PropertyName.SOLUTION, buildJsStringValue( solution ), INDENT2 );

      String path = dataSourceComp.tryGetPropertyValue( PropertyName.PATH, "" );
      addJsProperty( out, PropertyName.PATH, buildJsStringValue( path ), INDENT2 );

      String file = dataSourceComp.tryGetPropertyValue( PropertyName.FILE, "" );
      addJsProperty( out, PropertyName.FILE, buildJsStringValue( file ), INDENT2 );
    }
  }
  ...
  private void renderBuiltinCdaDatasource( StringBuilder out, CdfRunJsDashboardWriteContext context,
    ...
    addFirstJsProperty( out, PropertyName.DATA_ACCESS_ID, buildJsStringValue( dsName ), INDENT2 );
    ...
    addJsProperty( out, PropertyName.PATH, buildJsStringValue( cdeFilePath ), INDENT2 );
  ...
  private void renderCpkDatasource( StringBuilder out, DataSourceComponent dataSourceComp ) {
    ...
    addFirstJsProperty( out, PropertyName.ENDPOINT, buildJsStringValue( endpoint ), INDENT2 );

    String pluginID = compType.tryGetAttributeValue( PropertyName.PLUGIN_ID, "" );
    addJsProperty( out, PropertyName.PLUGIN_ID, buildJsStringValue( pluginID ), INDENT2 );

    String outputStepName = dataSourceComp
      .tryGetPropertyValueByName( PropertyName.KETTLE_OUTPUT_STEP_NAME, "OUTPUT" );
    addJsProperty( out, PropertyName.KETTLE_OUTPUT_STEP_NAME, buildJsStringValue( outputStepName ), INDENT2 );

    String outputFormat = dataSourceComp
      .tryGetPropertyValueByName( PropertyName.KETTLE_OUTPUT_FORMAT, "Infered" );
    addJsProperty( out, PropertyName.KETTLE_OUTPUT_FORMAT, buildJsStringValue( outputFormat ), INDENT2 );

    String queryType = PropertyValue.CPK_QUERY_TYPE;
    addJsProperty( out, PropertyName.QUERY_TYPE, buildJsStringValue( queryType ), INDENT2 );
  }
  ...
  private void renderDatasource( StringBuilder out, DataSourceComponent dataSourceComp ) {
    ...
    addFirstJsProperty( out, PropertyName.JNDI, buildJsStringValue( jndi ), INDENT2 );

    String catalog = dataSourceComp.tryGetPropertyValue( PropertyName.CATALOG, "" );
    addJsProperty( out, PropertyName.CATALOG, buildJsStringValue( catalog ), INDENT2 );

    String cube = dataSourceComp.tryGetPropertyValue( PropertyName.CUBE, "" );
    addJsProperty( out, PropertyName.CUBE, buildJsStringValue( cube ), INDENT2 );
    ...
    addJsProperty( out, PropertyName.QUERY, query, INDENT2 );
    addJsProperty( out, PropertyName.QUERY_TYPE, buildJsStringValue( queryType ), INDENT2 );
  }

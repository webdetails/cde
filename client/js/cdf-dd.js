...
var CDFDD = Base.extend({
		...
		init: function(){
			...
			$(".tooltip").tooltip({showURL: false });
                        ...
                        // Load styles list
			var myself = this;
			$.getJSON("SyncStyles", {operation: "listStyles"}, function(json) {
				...
				title: wcdf.title,author: wcdf.author,description: wcdf.description, style: wcdf.style};
					...
					$.notifyBar({ html: "Errors initializing settings: " + json.result });
                                    }
			        });
                         }
                },
		...
		load: function(){
			...
			var myself = this;
			var loadParams = {operation: "load",file: CDFDDFileName } 
		...
		save: function(){
			...
			if (CDFDDFileName != "/null/null/null") {
				$.post(CDFDDDataUrl, saveParams, function(result) {
					...
					var json = eval("(" + result + ")");
					if(json.status == "true"){
						$.notifyBar({ html: "Dashboard saved successfully", delay: 1000 });
					}
					else{
						$.notifyBar({ html: "Errors saving file: " + json.result });
					}
				});
			}
			else {
		...
		saveAs: function(fromScratch){
			...
			$.prompt(content,{
				loaded: function(){
					$('#fileInput').change(function(){selectedFile = this.value;});
					$('#titleInput').change(function(){selectedTitle = this.value;});
					$('#descriptionInput').change(function(){selectedDescription = this.value;});
					$('#container_id').fileTree(
						...
						function(obj,folder){if($(".selectedFolder").length > 0)$(".selectedFolder").attr("class","");$(obj).attr("class","selectedFolder"); selectedFolder = folder;$("#fileInput").val("")}}, 
						function(file) {$("#fileInput").val(file.replace(selectedFolder,""));selectedFile = $("#fileInput").val();});
				},
				...
				callback: function(v,m,f){
					if(v){
						...
						if(selectedFile.indexOf(".") != -1 && (selectedFile.length < 5 || selectedFile.lastIndexOf(".wcdf") != selectedFile.length-5))
							...
							var saveAsParams = {operation: fromScratch  ? "newFile" : "saveas", file: selectedFolder + selectedFile, title: selectedTitle, description: selectedDescription, cdfstructure: JSON.stringify(myself.dashboardData,"",2) };
		...
		reload: function(){
			...
			$.prompt('Are you sure you want to reload? Unsaved changes will be lost.',{ buttons: { Ok: true, Cancel: false} ,
				callback: function(v,m,f){
			...
			callback: function(v,m,f){
					if(v){
						...
						var saveSettingsParams = {
							operation: "saveSettings",
							file: CDFDDFileName.replace(".cdfde",".wcdf"), 
							title: title,author: author,description: description, style: style};
						
						$.post(CDFDDDataUrl, saveSettingsParams, function(result) {
							var json = eval("(" + result + ")");
							if(json.status == "true"){
								myself.setDashboardWcdf({title: title,author: author,description: description, style: style});
								$.notifyBar({ html: "Dashboard Settings saved successfully", delay: 1000 });
							}
							else
								$.notifyBar({ html: "Errors saving settings: " + json.result });
						});
					}
			}});
		},

		setDashboardData: function(dashboardData){this.dashboardData = dashboardData},
		getDashboardData: function(){return this.dashboardData},
		setDashboardWcdf: function(dashboardWcdf){this.dashboardWcdf = dashboardWcdf},
		getDashboardWcdf: function(){return this.dashboardWcdf}

	},
	{
		LAYOUT: function(){return $("table#layout-div tbody")},
		PANELS: function(){return $("#cdfdd-panels")}
	});
...
var Logger = Base.extend({
		...
		error : function(str){this.log(this.ERROR,str)},
		warn : function(str){this.log(this.WARN,str)},
		info : function(str){this.log(this.INFO,str)},
		debug : function(str){this.log(this.DEBUG,str)}
...
var CDFDDUtils = Base.extend({
		...
		ev: function(v){return (typeof v=='function'?v():v);},
...
$(function() {
		...
		$.editable.types.selectMulti = {
			...
			content : function(data, settings, original) {
				...
				$.each(_selectedArray,function(i,val){_selectedHash[val] = true});
				...
				$('select', this).multiSelect({oneOrMoreSelected: "*"},function() {});
			}
		};

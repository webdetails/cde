...
public class DependenciesManager {
  ...
  private Map<String, DependenciesEngine> engines;
    ...
    synchronized (syncObj) {
      ...
      if (_engine == null) {
        ...
        StringFilter jsFilter = new StringFilter() {
          ...
          public String filter(String input) {
            return "\t\t<script language=\"javascript\" type=\"text/javascript\" src=\"getJsResource/" + input
                + "\"></script>";
          }
        };
        ...
        _engine.registerEngine(Engines.CDF_CSS, new DependenciesEngine(Engines.CDF_CSS, cssFilter, basePath,
            Packager.Filetype.CSS));
        _engine.registerEngine(Engines.CDF, new DependenciesEngine(Engines.CDF, jsFilter, basePath,
            Packager.Filetype.JS));
        _engine.registerEngine(Engines.CDF_RAW, new DependenciesEngine(Engines.CDF_RAW, rawFilter, basePath,
            Packager.Filetype.JS));
        _engine.registerEngine(Engines.CDFDD, new DependenciesEngine(Engines.CDFDD, jsFilter, basePath,
            Packager.Filetype.JS));
      }
    }

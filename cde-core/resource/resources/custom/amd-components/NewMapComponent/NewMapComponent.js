...
define([
  ...
  './mapAddIns',
  'css!./NewMapComponent'],
  function(UnmanagedComponent, Logger, $, _, MapComponentAsyncLoader, GoogleMapEngine, OpenLayersEngine) {
  ...
  //(function() {
    ...
    var ColorMapMixin = {
      ...
      getColorMap: function() {
        ...
        var cmap = [];
        // convert string representation of arrays into array of arrays
        this.colormap = _.map(this.colormap, JSON.parse);
    ...
    var NewMapComponent = UnmanagedComponent.extend(ShapeConversionMixin).extend(ColorMapMixin).extend({
      ...
      load_deferred_hack: function() {
        ...
        var deferredQuery = function() {
          var _deferredQuery = $.Deferred(), myself = this;
        ...
        return $.when(
          this.dataRequest(this.shapeSource),
            deferredQuery()
      ...
      load_deferred: function() {
        ...
        if(this.mapEngine == 'google') {
          deferreds.push(MapComponentAsyncLoader('3',  this.API_KEY));
        }
          ...
          return _.bind(function(data) {
            ...
            if(counter >= this.runCounter) {
              try {
                ...
                Logger.error(this.dashboard.getErrorObj('COMPONENT_ERROR').msg, e);
                Logger.log(e,"error");
              }
            }
      ...
      getShapeDefinition : function(callback) {
        Logger.log('NewMapComponent.getShapeDefinition: entered with arguments ' + JSON.stringify(arguments), 'debug');
        var myself = this;
      ...
      processShapeDefinition : function(data, callback) {
        ...
        if(data)  {
          switch(filetype) {
            case 'kml':
              Logger.log('parsing shapes', 'debug');
        ...
        if(_.isFunction(callback)) {
          Logger.log('NewMapComponent.processShapeDefinition: callback is a function', 'debug');
          callback(data);
        }
      },
      ...
      renderMarker: function(location, elt, mapping, position) {
        ...
        if(location === undefined) {
          Logger.log('Unable to get location for address ' + elt[mapping.address] + '. Ignoring element.', 'debug');
          return true;
        }
        ...
        Logger.log('About to render ' + location[0] + ' / ' + location[1] + ' with marker ' + marker + ' sized ' + markerHeight + ' / ' + markerWidth + 'and description ' + description, 'debug');
      ...
      markerClickCallback: function(event) {
        ...
        $(this.popupParameters).each(function(i, eltA) {
          myself.dashboard.fireChange(eltA[1], event.data[mapping[eltA[0].toLowerCase()]]);
        });
      ...
      getMapping: function(values) {
        ...
        if(!values.metadata || values.metadata.length == 0) { return map; }

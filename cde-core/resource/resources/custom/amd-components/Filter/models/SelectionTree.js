...
define([
  ...
  BaseFilter.Models.SelectionTree = BaseFilter.Models.Tree.extend({
    ...
    setSelection: function(newState) {
      ...
      if (newState !== BaseFilter.Enum.select.SOME) {
        if (this.children()) {
    ...
    setSelectedItems: function(idList) {
      ...
      flatTree.filter(function(m) {
        ...
        if (_.contains(idList, id)) {
          return m.setSelection(BaseFilter.Enum.select.ALL);
        } else {
          return m.setSelection(BaseFilter.Enum.select.NONE);
        }
      });
      flatTree.filter(function(m) {
        return m.children() != null;
    ...
    _getSelectionSnapshot: function() {
      ...
      selectionSnapshot = {
        ...
        some: flatTree.filter(function(m) {
          return m.getSelection() === BaseFilter.Enum.select.SOME;
        }),
        all: flatTree.filter(function(m) {
          return m.getSelection() === BaseFilter.Enum.select.ALL;
        })
      };
    ...
    update: function() {
      ...
      this.root().updateCountOfItems('numberOfSelectedItems', function(model) {
        if (model.getSelection() === BaseFilter.Enum.select.ALL) {
          return 1;
        } else {
          return 0;
        }

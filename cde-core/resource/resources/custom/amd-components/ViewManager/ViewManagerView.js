...
define([
  ...
  var ViewManagerView = Backbone.View.extend({
    ...
    render: function() {
      //check if this.dashboard.view exists and if there is a model saved with the same name
      var viewName = this.model.get('dashboard').view ? this.model.get('dashboard').view.name : undefined,
          viewDesc = this.model.get('dashboard').view ? this.model.get('dashboard').view.description : undefined;
    ...
    renderViewList: function() {
      ...
      this.collection.each(function(model) {
        var $view = $(templates.viewListItem({
          name: model.get("name"),
          viewUrl: ViewManagerComponentExt.getUrl(
            model.get("solution"),
            model.get("path"),
            model.get("file"),
            model.get("name"))}));
    ...
    saveView: function() {
      ...
      if(!name || !name.length || name === "Unsaved") {
        Logger.warn("Failed to save a dashboard view with no name or with the reserved name 'Unsaved'");
        return;
      }

      

      var tmpModel = dashboard.view
        ? new ViewModel(dashboard.view)
        : new ViewModel();

      tmpModel.set("name", name, {silent: true});
      tmpModel.set("description", desc, {silent: true});
      // OrientDB needs param to be base64 encoded
      tmpModel.set("params", Base64.encode(JSON.stringify(dashboard.getViewParameters())), {silent: true});
      tmpModel.set("unbound", dashboard.getUnboundParameters(), {silent: true});
      tmpModel.set("user", dashboard.context.user, {silent: true});
      tmpModel.set("solution", dashboard.context.solution, {silent: true});
      tmpModel.set("path", dashboard.context.path, {silent: true});
      tmpModel.set("file", dashboard.context.file, {silent: true});
      ...
      tmpModel.sync(
        ...
        tmpModel,
        {
          url: ViewManagerComponentExt.getViewsEndpoint(name),
          ...
          success: function(response) {
            ...
            dashboard.view = response.result;

            dashboard.restoreView();

            myself.collection.add(dashboard.view, {merge: true, silent: false});
          },
          cache: false
        }
      );
    ...
    deleteView: function(evt) {
      ...
      if(!name || !name.length) {
        Logger.warn("Failed to delete a dashboard view with no name");
        return;
      }
      ...
      tmpModel.sync(
        ...
        tmpModel,
        {
          ...
          success: function(response) {
            ...
            // if we just deleted the current this.dashboard.view previously saved
            // we need to remove the id so an update on a deleted view is not triggered
            if(dashboard.view && dashboard.view.name === name) {
              dashboard.view.id = null;
            }
          },
          cache: false
        }
